...
export default function ClaudeUsagePanel({ data, error, connected, lastUpdate, api, config, cls }) {
  ...
  const handleRefresh = async () => {
    setRefreshing(true);
    try {
      await api.fetch('/api/usage/refresh', { method: 'POST' });
    } catch {}
    ...
    <div class=${cls('wrap')}>
      ...
      <div class=${cls('header')} style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px">
        ...
        <span style="font-size:10px;color:var(--text-dim);font-family:'JetBrains Mono',monospace;display:flex;align-items:center;gap:4px">
          ...
          <button onClick=${handleRefresh} style="background:none;border:none;cursor:pointer;font-size:14px;opacity:${refreshing ? 0.3 : 0.6};padding:2px 4px" disabled=${refreshing}>🔄</button>
        </span>
      </div>
